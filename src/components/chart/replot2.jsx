        ...
        try {
          if(this.historySeriesPair != this.treatPair(this.pair) || true) {
            ...
            for (let x = 0; x < data.timestamp.length; x++) {
              let no_data = data.indicators.quote[0].open[x] && data.indicators.quote[0].high[x] && data.indicators.quote[0].low[x] && data.indicators.quote[0].close[x];
              if(no_data) {
                xdata = {
                ...
                plot = this.graphData2(xdata, this.pair);
                xdata = plot;
              ...
              if(xdata && !no_data) {
                plot = xdata;
                plot.Date = data.timestamp[x];
                plot = this.graphData2(plot, this.pair);
                
                this.historySeries.push(plot);
                this.historySeriesPair = this.treatPair(this.pair);
                this.plotGraph(plot);
                console.log("Saved", data.timestamp[x]);
              }
            }
