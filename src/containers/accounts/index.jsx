              ...
              <ul className={"t-history-record"+(key == 0 ? " _active" : "")} id={"t-history-record-"+key} onClick={(e) => this.handleClick(e, 't-history-record-'+key)}>
                ...
                <li>
                  ...
                  <span className="td">
                    ...
                    <svg className="acc-gra pointer" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => { $("#t-history-record-alt-"+key).toggleClass("hide"); }}>
                      <path d="M0.25 5.5H4V17.75H0.25V5.5ZM7.25 0.25H10.75V17.75H7.25V0.25ZM14.25 10.25H17.75V17.75H14.25V10.25Z" fill="#1FCF65"/>
                    </svg>
                  </span>
                </li>
              </ul>
              <AccountStat id={"t-history-record-alt-"+key} altid={(Number(key) + 1)} account={acc.account_name} type={acc.account_type.toUpperCase()} label={acc.account_label.length ? acc.account_label : "AV-"+acc.account_id} />
            </>
          ))}
