...
class Admins extends Component {
  ...
  render () {
    let { page_no, page_size, admins, filter, soft_delete, utype } = this.state;
      ...
      <Spinner showSpinner={this.state.showLoader} />
        <div className="col-12" id="users-container">
          <div className="users-section-right">
            ...
            <TableFilters table="admins" sd={(e) => this.setState({soft_delete: e.target.value, page_no: 1})} search={(e) => this.setState({filter: e.target.value})} />

            <ConfirmModal
              head={(this.state.restore ? "Restore" : "Delete")+" this admin?"}
              ...
              admins.map((av, idx) => (
                <ul className="table-body">
                  ...
                  <li className="shot">
                    ...
                    {av.soft_delete > 0 ? (
                        <img src={check} onClick={() => this.setState({confirmID: av.user_id, confirmModal: true, restore: true})} style={{height: "19px", width: "20px", marginLeft: "10px", cursor: "pointer"}} />
                      ) : (
